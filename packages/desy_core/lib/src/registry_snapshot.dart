...
/// JSON-ready description derived from one complete [DesyRegistry].
///
...
class DesyRegistrySnapshot {
  /// Creates a derived portable snapshot for [registry].
  const DesyRegistrySnapshot(this.registry);

  /// The consumer registry that remains this export's only source of truth.
  final DesyRegistry registry;

  /// A JSON-serializable representation of the declared registry.
  Map<String, Object?> toJson() {
    final identity = registry.identity;
    if (identity == null) {
      throw StateError(
        'Add DesyRegistryIdentity to the registry before exporting a '
        'portable snapshot.',
      );
    }
    return {
      'schemaVersion': 'desy-registry-snapshot/0.1',
      'registry': {
        'id': identity.id,
        ...
        'assets': [
          for (final asset in registry.allAssets)
            _entry(
              ...
              extra: {'assetKey': asset.assetKey, 'mimeType': asset.mimeType},
            ),
        ],
      },
      'components': [
